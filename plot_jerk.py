...
for n in trips:
    ...
    for file in files:
        ...
        jerk = np.diff(filteredData) / dt
        plt.plot(uniform_time[1:], jerk)
    ...
    plt.ylabel('Jerk X')
    plt.title(f'Trip {n}')
    plt.savefig(f'plots/jerk_trip{n}.png', bbox_inches='tight', dpi=300)
    plt.close()
...
for i, ax in enumerate(axes):
    ...
    for file in files:
        ...
        filteredData = signal.filtfilt(b, a, uniform_data)
        jerk = np.diff(filteredData) / dt
        ax.plot(uniform_time[1:], jerk)

    ax.set_ylim([y_lo, y_hi])
    ax.set_xlabel('Time')
    ax.set_ylabel('Jerk X')
    ax.set_title(f'Trip {n}')

plt.savefig(f"plots/jerk_trip{''.join(trips)}.png", bbox_inches='tight', dpi=300)
plt.close()